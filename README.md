...
- 家人管理仅位于首次选择成员的弹窗中，首次使用默认密码 `123456`。
- 进入成员管理后可修改家庭管理密码。新密码使用 6–12 位数字，并拒绝默认密码、连续数字和重复数字；修改后同一家庭的所有设备同步生效。
- 任务类型可在任务管理中维护。
...
2. 必须先创建当天父额度，提供 60、120、180、185 分钟和自定义分钟数。
...
flowchart TB
  ...
  STORE --> PASSWORD[受限密码校验与修改 RPC]
  DB --> RLS[RLS 家庭访问控制]
  PASSWORD --> SECRET[private schema\n加盐哈希密码]
```
...
- 家庭管理密码只以 bcrypt 加盐哈希保存在 `private` schema；浏览器通过受限 RPC 校验和修改，不能读取密码或哈希。
...
| 启用可修改的家庭管理密码 | [management-password-migration.sql](supabase/management-password-migration.sql) |
| 为已加入家庭的设备启用“退出家庭” | [leave-household-migration.sql](supabase/leave-household-migration.sql) |
| 新家庭邀请码持续“同步失败”或退出 RPC 不可用 | [family-sync-recovery-migration.sql](supabase/family-sync-recovery-migration.sql) |

`ipad-usage-migration.sql` 是 iPad 功能必需迁移：未执行时，页面可以显示但 iPad 类型、额度和使用记录无法可靠同步到云端。

已有线上项目升级时，必须先执行 `management-password-migration.sql`，确认 SQL 成功后再发布前端。否则新版页面无法校验或修改家庭管理密码。脚本会为每个现有家庭初始化默认密码 `123456` 的加盐哈希，用户进入成员管理后可自行修改。

`leave-household-migration.sql` 用于已运行中的项目：它会允许通过邀请码加入的设备撤销自己的云端访问权限，不会删除家庭、任务或成员数据。

如果线上项目出现“新家庭邀请码同步失败”，在 SQL Editor 整段执行 `family-sync-recovery-migration.sql`。该脚本会修复 `pgcrypto` 所在 schema、家庭管理密码触发器、新家庭写入策略和退出家庭 RPC；脚本可重复执行，不会删除已有家庭、成员、任务或完成记录。执行成功后回到成员管理点击“重新同步”。
...
│   ├── management-password.js      # 管理密码输入规则
...
│   ├── management-password-migration.sql # 管理密码迁移
│   ├── leave-household-migration.sql # 退出家庭迁移
│   └── family-sync-recovery-migration.sql # 家庭同步与退出修复
└── tests/                         # Node 内置测试
```
